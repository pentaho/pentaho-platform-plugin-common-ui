...
angular.module('pascalprecht.translate').provider('$translatePartialLoader', function () {
  ...
  Part.prototype.getTable = function (lang, $q, $http, urlTemplate, errorHandler, fileFormatAdapter) {
    ...
    if (!this.tables[lang]) {
      ...
      $http({
        ...
        if (fileFormatAdapter)
          data = fileFormatAdapter(data);
    ...
    function ($rootScope, $injector, $q, $http) {
      var service = function (options) {
        ...
        if (fileFormatAdapter !== undefined && !angular.isFunction(fileFormatAdapter)) {
          throw new Error('Unable to load data, a fileFormatAdapter is not a function!');
        }
        ...
        for (var part in parts) {
          if (hasPart(part) && parts[part].isActive) {
            loaders.push(parts[part].getTable(options.key, $q, $http, options.urlTemplate, errorHandler, fileFormatAdapter).then(addTablePart));
          }
        }
        if (loaders.length) {
