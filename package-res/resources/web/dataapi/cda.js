	...
	discoverDescriptors: function(func){
		...
		if (pentaho.cda.descriptors.length > 0) {
			...
			func(pentaho.cda.descriptors);
				...
				pentaho.xhr.execute(CONTEXT_PATH + "content/cda/getCdaList", {
					async: true,
					dataType: "json",
					type: "GET",
...
pentaho.cda.Descriptor.prototype = {
	...
	,discoverQueries: function(func) {
		...
		if (that.queries.length == 0) {
			pentaho.xhr.execute(CONTEXT_PATH + "content/cda/listQueries", {
				async: true,
				dataType: "json",
				type: "GET",
				data: {
...
pentaho.cda.Query.prototype = {
	discoverParameters: function(func){
		...
		if (this.parameters.length == 0) {
			$.getJSON(CONTEXT_PATH + "content/cda/listParameters", {path:that.file.path, dataAccessId:this.id},
			function(data){
	...
	$.getJSON(CONTEXT_PATH + "content/cda/doQuery", {path:this.file.path, dataAccessId:this.id},
		function(data){
			//console.log(columns);
			if (typeof func == 'function'){
				func(data);
