...
define([ 'cdf/lib/jquery', 'cdf/dashboard/Utils', 'common-ui/prompting/components/CompositeComponent' ], function($,
  ...
  describe("CompositeComponent", function() {
    ...
    describe("mapComponents", function() {
      var controlParamValue = "executed_test_callback";
      var comp;
      var testParam;
      var testFn;
      beforeEach(function() {
        testFn = function(c) {
          testParam = controlParamValue;
        };
        comp = new CompositeComponent();
        spyOn(comp, "mapComponentsList");
      });

      it("should map components", function() {
        var compObj = {
          components : [ jasmine.createSpy('component') ]
        };
        var obj = comp.mapComponents(compObj, testFn, this);

        expect(obj).toBe(compObj);
        expect(testParam).toBe(controlParamValue);
        expect(comp.mapComponentsList).toHaveBeenCalledWith(compObj.components, testFn, this);
      });

      it("should not map components", function() {
        var compObj = {};
        var obj = comp.mapComponents(compObj, testFn, this);

        expect(obj).toBe(compObj);
        expect(testParam).toBe(controlParamValue);
        expect(comp.mapComponentsList).not.toHaveBeenCalled();
      });
    });

    describe("mapComponentsList", function() {
      var comp;
      var testFn;
      beforeEach(function() {
        testFn = function(c) {
        ...
        comp = new CompositeComponent();
        spyOn(comp, "mapComponents");
      });

      it("should map components list", function() {
        var intComp = jasmine.createSpy('component');
        var components = [ intComp ];
        var obj = comp.mapComponentsList(components, testFn, this);

        expect(obj).toBe(comp);
        expect(comp.mapComponents).toHaveBeenCalledWith(intComp, testFn, this);
      });

      it("should not map components list", function() {
        var obj = comp.mapComponentsList([], testFn, this);

        expect(obj).toBe(comp);
        expect(comp.mapComponents).not.toHaveBeenCalled();
      });
    });
  });
});
