...
define([
  ...
  /*global DataTable:true, sort:true, dojo:true, visualizations:true, currentView:true*/
  ...
  pentaho.palettes.push(
    colorPalettes.get("palette 1"),
    colorPalettes.get("palette 2"),
    colorPalettes.get("palette 3"));

  // ---------

  pentaho.visualizations = pentaho.visualizations || [];

  visualizations = pentaho.visualizations;
  ...
  pentaho.VizController.prototype.doVisualization = function( visualization, userDefinedOptions ) {
    ...
    if(typeof console !== 'undefined') {
      var m = console.warn ? "warn" :
              console.log  ? "log"  : null;
      if(m) {
        console[m](
          "[WARN][VizAPI] Rendering legacy visualization '" + visualization.id +
  ...
  // ??
  sort = function sort( columnIdx, direction ) {
  ...
  pentaho.VizController.createPaletteMap = colorUtils.createPaletteMap;

  pentaho.VizController.getRrbGradient = function(value, min, max, color1, color2) {
    return colorUtils.getRgbGradient(value, min, max, color1, color2);
  };
  pentaho.VizController.getRgbGradientFromMultiColorHex = function(value, min, max, colors) {
    return colorUtils.getRgbGradient(value, min, max, colors);
  };

  pentaho.VizController.getDarkerFromColorHex = colorUtils.getRgbDarker;

  pentaho.VizController.getRgbStepFromMultiColorHex = colorUtils.getRgbStep;

  pentaho.VizController.convertToRGB = colorUtils.parseColor;
  pentaho.VizController.getRrbColor  = colorUtils.getRgbColor;
