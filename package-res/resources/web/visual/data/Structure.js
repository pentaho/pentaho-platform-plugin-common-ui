...
define([
  ...
  "./Collection",
  "../_utils"
], function(Attribute, StructurePosition, Collection, utils) {

  // keyArgs: model
  function Structure(keyArgs) {
    this._model = utils.required(keyArgs, "model", "keyArgs");

    Collection.call(this, keyArgs);
  }

  utils.inherit(Structure, Collection, {
    ...
    toSpec: function(keyArgs) {
      return this.map(
          utils.option(keyArgs, "shareModel", false)
  ...
  // keyArgs: model
  Structure.to = function(structSpec, keyArgs) {
    if(structSpec != null) {
      if(structSpec instanceof Structure) return structSpec;
      if(!(structSpec instanceof Array)) throw utils.error.argInvalid("structSpec", "Not an array.");
    }

    return utils.setClass(structSpec || [], Structure, keyArgs);
  };
  ...
  StructurePosition.to = function(spec, keyArgs) {
    ...
    if(!attr) throw utils.error.argRequired("spec.attr");

    var structPos = Object.create(attr.structurePositionBase);
