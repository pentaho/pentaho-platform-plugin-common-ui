...
define([
  ...
  "./_cross/Table",
  "../_utils"
], function(AbstractDataTable, DataModel, PlainTable, CrossTable, utils) {
   ...
   * require the module `"pentaho/visual/data/Table"`.
   *
   * ### Remarks
   *
   * A `Table` can be constructed empty, or from either:
   * * a {@link pentaho.visual.data.AbstractTable} object,
   ...
   * by using the {@link pentaho.visual.data.Table#toPlainTable} method.
   *
   * @example <caption>Creating a table from a table specification</caption>
   *
   *     require("pentaho/visual/data/Table", function(Table) {
   ...
   * @class
   * @extends pentaho.visual.data.AbstractTable
   *
   * @param {string|pentaho.visual.data.ITableSpec|pentaho.visual.data.AbstractTable} [table]
   *    An instance of {@link pentaho.visual.data.AbstractTable},
   *    a table specification, or
   *    a JSON specification string.
   *
   * @amd pentaho/visual/data/Table
   */
  function Table(table) {
    ...
    this.model = DataModel.to(modelSpec);
    this.implem = new TableImplemClass(tableSpec, {model: this.model});
  }

  utils.inherit(Table, AbstractDataTable, /** @lends pentaho.visual.data.Table# */{
    /** @inheritdoc */
    ...
    _readTableArgument: function(table) {
      ...
      if(table instanceof AbstractDataTable) return table.toSpec();
     ...
     * {@link pentaho.visual.data.Table#toSpecPlain},
     * with the same keyword arguments,
     * and is returned.
     *
     * @return {pentaho.visual.data.Table} A table with a plain structure.
     * @see pentaho.visual.data.Table#toSpecPlain
     */
     ...
     * {@link pentaho.visual.data.Table#toSpec}
     ...
     * @return {pentaho.visual.data.IPlainTableSpec} A plain-structure specification of the table.
     * @see pentaho.visual.data.Table#toPlainTable
     */
     ...
     *    the model object itself and not its specification.
     *
     * @return {pentaho.visual.data.ITableSpec} A specification of the table.
     */
    ...
    _toSpec: function(keyArgs, asPlain) {
      ...
      tableSpec.model = utils.option(keyArgs, "shareModel") ? this.model : this.model.toSpec();
      return tableSpec;
    }
  });
   ...
   *   });
   *
   * @memberOf pentaho.visual.data.Table
   * @method
   * @param {Object} cdaTable A table object in _CDA_ format.
   * @return {Object} A table object in _data table_ format.
   * @see pentaho.visual.data.AbstractTable#toJsonCda
   */
  Table.convertJsonCdaToTableSpec = function(cdaTable) {
