...
define([
  ...
  "./MeasureCellSet",
  "../../_utils"
], function(Structure, Axis, MeasureCellSet, utils) {

  // keyArgs: model
  function Table(spec, keyArgs) {
    /*
    ...
    var model = utils.required(keyArgs, "model", "keyArgs"),
        laySpec = utils.option(spec, "layout"),
        ka = {model: model},
        lay = {
  ...
  utils.implement(Table, {
    ...
    _hasMicCol: function(colOrdinal, meaStructPos) {
      ...
      return utils.O_hasOwn.call(this._micColsIndex, micKey);
    },
    ...
    _addCrossCol: function(colSpec, j) {
      ...
      if(!colSpec) throw utils.error.argRequired("cols[" + j + "]");
      ...
      var meaAttrName;
      if(xC) {
        if(typeof colSpec !== "object") throw utils.error.argInvalid("cols[" + j + "]", "Must be an object.");

        meaAttrName = colSpec.attr;
      } else {
        if(!xM)
          throw utils.error.argInvalid("cols[" + j + "]",
      ...
      if(!xM) {
        if(meaAttrName != null)
          throw utils.error.argInvalid("cols[" + j + "].attr", "Cannot be specified when there are no measures.");

        meaStructPos = null;
      } else if(meaAttrName == null) {
        if(xM > 1)
          throw utils.error.argInvalid("cols[" + j + "].attr", "Required when there is more than one measure.");
      ...
      if(this._hasMicCol(colOrdinal, meaStructPos))
        throw utils.error.argInvalid("cols[" + j + "]", "Duplicate column cell tuple and measure attribute.");
    ...
    _addRow: function(rowSpec, i) {
      if(!rowSpec) throw utils.error.argRequired("rows[" + i + "]");
      ...
      if(rowSpec instanceof Array) {
        ...
        if(!(rawRowCellSpecs instanceof Array))
          throw utils.error.argInvalid("rows[" + i + "].c", "Not an array.");
      }
      ...
      if(i !== r)
        throw utils.error.argInvalid("rows[" + i + "].c", "Duplicate row tuple.");
