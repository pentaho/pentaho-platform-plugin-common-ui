...
define([
  "../Structure",
  "../../_utils"
], function(Structure, utils) {

  // spec: [Array(Array(*))]
  // keyArgs: structure, [C: 0]
  function MeasureCellSet(spec, keyArgs) {
    this._structure = utils.required(keyArgs, "structure", "keyArgs");
  ...
  utils.implement(MeasureCellSet, {
  ...
  function MeasureCube(keyArgs) {
    // row -> col -> {<meaName>: cell, ...} (multiple measures)
    // row -> col -> cell (single measure)

    this._C = utils.option(keyArgs, "C", 0);
    this._cube = [];
  }

  utils.implement(MeasureCube, {
  ...
  function MultiMeasureCube(keyArgs) {
    var structure = utils.required(keyArgs, "structure", "keyArgs");

    MeasureCube.call(this, keyArgs);

    var measHolderBase = this._measHolderBase = {};
    structure.forEach(function(pos) { measHolderBase[pos.attribute.name] = null; });
  }

  utils.inherit(MultiMeasureCube, MeasureCube, {
  ...
  function SingleMeasureCube(keyArgs) {
    MeasureCube.call(this, keyArgs);
  }

  utils.inherit(SingleMeasureCube, MeasureCube, {
