...
define([
  ...
  "es6-promise-shim"
], function(utils, typeProviders, apiConfigs) {
  ...
  utils.extend(VisualTypeRegistry.prototype, {
    /**
     * Adds a visual type.
     *
    ...
    add: function(type) {
      ...
      if(!type.id) throw utils.error.argRequired("type.id");

      var baseEntry = this._baseEntry,
          type0  = utils.O_getOwn.call(baseEntry.map, type.id, null);

      if(type0) {
        if(type0 === type) return this;

        throw utils.error.argInvalid(
            "type",
    ...
    get: function(typeId, containerTypeId, assertAvailable) {
      if(!typeId) throw utils.error.argRequired("typeId");

      var map = getContainerEntry.call(this, containerTypeId).map,
          type = utils.O_getOwn.call(map, typeId, null);
    ...
    addConfig: function(config) {
      if(!config) throw utils.error.argRequired("config");
      // Already added?
      ...
      var ids = config.id,
          ...
          processId = function(id) {
            if(id == null || id === "") {
              ...
              hasAllGroup = true;
              config._matchGroupId = utils.F_true;
  ...
  function getConfigLevel(priority) {
    var configLevel = utils.O_getOwn.call(this._configLevelsMap, priority);
    if(!configLevel) {
      ...
      this._configLevelsList.sort(function(ca, cb) {
        return utils.compare(ca.priority, cb.priority);
      });
    }
  ...
  function getContainerEntry(containerTypeId) {
    ...
    return (cache && utils.O_getOwn.call(cache, containerTypeId || "")) ||
  ...
  function cloneVisualType(baseVisualType) {
    ...
    for(var p in baseVisualType) {
      if(utils.O_hasOwn.call(baseVisualType, p)) {
  ...
  function applyConfig(type, config) {
    ...
    var v, p;
    for(p in config) {
      if( utils.O_hasOwn.call(config,          p) && // an own property of config
         !utils.O_hasOwn.call(lockedTypeProps, p) && // not locked
         (v = config[p]) !== undefined) {                  // not undefined

        // Replace, not merge.
        type[p] = utils.shallowObjectClone(v);
      }
    }
    ...
    if(config.args) {
      ...
      utils.O_copyOwnDefined.call(type.args, config.args);
    }
  ...
  function invalidateContainerCache(containerTypeIds) {
    ...
    function invalidateOne(c) {
      if(!c) c = "";
      var entry = utils.O_getOwn.call(cache, c);
    ...
    if(anyDeleted) {
      ...
      for(var c1 in cache)
        if(utils.O_hasOwn.call(cache, c1))
          return;
