...
[execute]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.action.Execute'}}) and
[select]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.action.Select'}}).
Most container applications handle these in some useful way.

## On data actions and filters...

Visualization API 
[data actions]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.action.mixins.Data'}}) 
...
[dataFilter]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.action.mixins.Data' | append: '#dataFilter'}})
property.
...
Remarks:
  - The scene object knows how to create a filter for the data it represents
    (see [createFilter]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.scene.Base' | append: '#createFilter'}})). 
...
Remarks:
  ...
  - The filter's 
    [$contentKey]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.data.filter.Abstract' | append: '#$contentKey'}})
...
[selectionFilter]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.base.Model' | append: '#selectionFilter'}})
property.
For each `Select` action that is performed, 
its [dataFilter]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.action.Select' | append: '#dataFilter'}}), 
may be removed from, be added to, replace or toggled in the model's current `selectionFilter`, 
according to the action's 
[selectionMode]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.action.Select' | append: '#selectionMode'}}).
...
Remarks:
  - Each time a bar is clicked, the current model's `selectionFilter` will be 
    [replaced]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.action' | append: '#.SelectionModes'}})
    with the data filter associated with the clicked bar, or 
    [toggled]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.action' | append: '#.SelectionModes'}})
...
Remarks:
  ...
  - The `Select` action's 
    [default action]({{site.refDocsUrlPattern82 | replace: '$', 'pentaho.visual.action.Select' | append: '#_doDefault'}})
