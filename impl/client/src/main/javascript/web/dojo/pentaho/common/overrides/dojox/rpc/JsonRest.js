define("dojox/rpc/JsonRest", ["dojox/json/ref", "dojox/rpc/Rest", "dojo/on", "dojo/_base/array", "dojo/aspect", "dojox/data/JsonRestStore"], function(ref, Rest, on, array, aspect, _restStore) {
	...
	function resolveJson(service, deferred, value, defaultId){
		...
		if(hrefProperty){
			ref.refAttribute = hrefProperty;
		}
		value = value && ref.resolveJson(value, {
			defaultId: defaultId,
			index: Rest._index,
	...
	jr = dojox.rpc.JsonRest={
      ...
      aspect.after(kwArgs,"onError",function(){
				if(kwArgs.revertOnError !== false){
					...
					array.forEach(savingObjects, function(obj) {
      ...
      // Dojo 2.0, do we need to extend the dojo.request.xhr provider?
      ...
      // End Dojo 2.0
			...
			for(i =0; i < actions.length;i++){ // iterate through the actions to execute
				...
				var dfd = action.deferred = service[action.method](
									...
									ref.toJson(action.content, false, service.servicePath, true)
								);
...
//			dojo.xhr = plainXhr;
		...
		_loader: function(callback){
			...
			var self = this;
			jr.query(serviceAndId.service, serviceAndId.id).addCallback(function(result){
		...
		isDirty: function(item, store){
			...
			if(!item){
				if(store){
					return array.some(dirtyObjects, function(dirty){
