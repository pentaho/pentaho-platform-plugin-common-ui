  ...
  return BaseView.extend(/** @lends pentaho.ccc.visual.Abstract# */{
    ...
    _updateSelection: function() {
      ...
      try {
        ...
        // There's a similar construct in Analyzer cv.Report#displayClientSideReport / change action init handler
    ...
    _configureContinuousColors: function() {
      ...
      if(paletteQuantitative) {
        ...
        this.options.colors = visualColorUtils.buildPalette(model.colorSet, model.pattern, model.reverseColors);
      }
    },
    ...
    _executeOn: function(cccContext) {
      ...
      executeFilter = new OrFilter({operands: [executeFilter]});
    ...
    _convertCccComplexToFilter: function(cccComplex) {
      ...
      this._getSourceKeyMappingFieldInfos().each(function(mappingFieldInfo) {
        ...
        if(cccAtom) {
          ...
          var operand = new IsEqualFilter({
            property: mappingFieldInfo.sourceName,
            value: value === null ? null : {_: valueType, v: value, f: cccAtom.label}
          });

          filter = filter !== null ? filter.and(operand) : new AndFilter({operands: [operand]});
        }
      });
