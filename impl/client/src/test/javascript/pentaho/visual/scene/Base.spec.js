...
define([
  ...
  describe("pentaho.visual.scene.Base", function() {
    ...
    describe("#removeChild(this) - child becoming a root", function() {

      // impact in `root`, `view` and `vars`
      ...
      it("should have #root be itself", function() {
        ...
        parentScene.removeChild(childScene);

        // ---

        expect(childScene.root).toBe(childScene);
        expect(childScene.parent).toBe(null);
        expect(childScene.index).toBe(-1);
      });

      it("should have #view be the old view", function() {
      ...
      it("should have #vars have a null prototype", function() {

        parentScene.removeChild(childScene);

        // ---

        expect(Object.getPrototypeOf(childScene.vars)).toBe(null);
      });

      it("should have #vars preserve its local variables", function() {

        childScene.setVar("category", "Portugal");
        childScene.setVar("measure", 6000);

        var categoryVar = childScene.vars.category;
        var measureVar = childScene.vars.measure;
        ...
        expect(childScene.vars.category).toBe(categoryVar);
        expect(childScene.vars.measure).toBe(measureVar);
      });

      it("should have #vars of grand-children inherit from its #vars", function() {

        var grandChild = new Scene(childScene);
        ...
        expect(Object.getPrototypeOf(grandChild.vars)).toBe(childScene.vars);
      });

      it("should have #vars of grand-children preserve their local variables", function() {

        var grandChild = new Scene(childScene);

        grandChild.setVar("series", "Lisbon");

        var seriesVar = grandChild.vars.series;
        ...
        expect(grandChild.vars.series).toBe(seriesVar);
      });
    });

    describe("#appendChild(this-a-root) - root becoming a child", function() {
      ...
      beforeEach(function() {
        view1 = new CustomView();
        view2 = new CustomView();

        parentScene = new Scene(null, view1);
        childScene = new Scene(parentScene);

        // Initially a root.
        grandChildScene = new Scene(null, view2);
      });

      it("should have #root be the new parent's root", function() {

        childScene.appendChild(grandChildScene);

        // ---

        expect(grandChildScene.root).toBe(parentScene);
      });

      it("should have #parent be the new parent", function() {

        childScene.appendChild(grandChildScene);

        // ---

        expect(grandChildScene.parent).toBe(childScene);
        expect(grandChildScene.index).toBe(0);
      });

      it("should have #view be the new parent's view", function() {

        childScene.appendChild(grandChildScene);

        // ---

        expect(grandChildScene.view).toBe(view1);
      });
      ...
      it("should have #vars have the parent's #vars as prototype", function() {
        ...
        expect(Object.getPrototypeOf(grandChildScene.vars)).toBe(childScene.vars);
      });

      it("should have #vars preserve its local variables", function() {

        childScene.setVar("category", "Lisbon");
        ...
        expect(grandChildScene.vars.category).toBe(categoryVar);
      });
    });

    describe("#appendChild(this-a-child-of-another-parent) - changing parents", function() {
      ...
      beforeEach(function() {
        ...
        parentScene1 = new Scene(null, view1);
        parentScene2 = new Scene(null, view2);

        childScene = new Scene(parentScene1);
      });

      it("should have #root be the new parent's root", function() {

        parentScene2.appendChild(childScene);

        // ---

        expect(childScene.root).toBe(parentScene2.root);
      });
      ...
      it("should have #view be the new parent's view", function() {
        ...
        expect(childScene.view).toBe(view2);
      });

      it("should have #vars have the new parent's #vars as prototype", function() {

        parentScene2.appendChild(childScene);

        // ---

        expect(Object.getPrototypeOf(childScene.vars)).toBe(parentScene2.vars);
      });
